...
- **MFA enforcement (CONSOLE-ONLY — no CLI/API):** Identity Center → Settings → Authentication → Multi-factor authentication → Configure. Confirmed 2026-06-23 there is no sso-admin/identitystore API for the human MFA sign-in policy (only unrelated `*-application-authentication-method` ops exist) — so this must be driven through the GUI; don't look for a CLI command. **Standing policy (David, 2026-06-23): context-aware, instance-wide** — "Prompt users for MFA = Only when sign-in context changes (context-aware)" (chosen over always-on to avoid re-prompting David's routine `aws sso login`), "Require them to register an MFA device at sign-in", authenticator apps + passkeys enabled, users self-manage devices. Rationale: instance-wide setting can't separate GUI vs CLI (both auth through the same portal), but context-aware + token caching means David's CLI is near-friction-free while Michael/Laura/Jessica are forced to enrol+use MFA on the GUI. This is the GitHub-org-2FA equivalent: all human access is via SSO, so login-MFA = MFA on every action incl. Bedrock changes; AWS has no per-service MFA toggle. Known bypass = the static `BedrockAPIKey-yx0m` key (invoke/read only, can't make config changes); acceptable. An Organizations SCP denying `aws:MultiFactorAuthPresent=false` is the stronger option but David has deferred it (lockout risk).
